...
const Chat = () => {
  ...
  const [chatEndedMessageDisplayed, setChatEndedMessageDisplayed] = useState(false);
  ...
  }, [chatStarted, chatEnded, timer, chatEndedLocally, chatEndedMessageDisplayed]);
  ...
  const handleSendMessage = () => {
    ...
    if (message.trim() !== "") {
      socket.emit(`${userId}-${professionalId}-chat`, { name, message, sent: true});
 ...
 <div className='chatPage'>
   <div className="chatHead">
       <div className="profileContent">
           ...
           <div className="profileInfo">
               ...
               <span>Balance: ({Math.floor(timer / 60).toString().padStart(2, '0')}:{(timer % 60).toString().padStart(2, '0')} mins)</span>
   ...
   <div className="rechargeInfo">
       <div className="tipInfo">
           ...
           <p><i className="bi bi-lightbulb-fill"></i> Tip: 90% users recharge for 10min or more</p>
       </div>
     ...
     <div ref={chatContainerRef} className="message-container">
       ...
       <List>
         {uniqueArray.map((msg, index) => (
            <div key={msg.id || index} className={`message ${msg.sent ? 'sent' : 'received'}`}>
